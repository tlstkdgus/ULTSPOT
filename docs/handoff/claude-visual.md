...
### `spot-card.tsx` — 날짜 확정 전 "이날 운영" 배지 (critique 4회차 P0의 나머지)

2단계는 날짜로 거르지 않는데(`trip-planner.tsx:89`, `step > 1`), 카드 배지가 `spotStatus(event, date)`로 기본 날짜를 받아 "이날 운영"/"이날 휴무"를 찍는다. 제목·배너·빈 상태의 날짜 언급은 T-034에서 뺐지만, 배지는 `SpotCard`의 계약을 바꿔야 해서 남겼다.

- **요청:** `date`가 확정되기 전(2단계)에는 운영/휴무 배지를 숨기고 날짜 무관 사실(운영시간·미확인·예약 필요)만 보여 달라. `date?: string`으로 바꾸거나 `dateConfirmed` 플래그를 넘기면 된다.
- **왜:** 사용자가 정하지 않은 날짜에 대해 검증하지 않은 운영 여부를 단언하는 것은 지어낸 운영시간과 같은 부류다. 월요일에 열면 HiKR Ground(`closedDays: [1]`)가 아무도 고르지 않은 날에 "휴무"로 찍힌다.
