...
> **T-011 현재 UI:** Your spots 상단에서 아티스트 검색·최대 5개 선택 또는 Explore all K-pop을 이용한다. 초기 목록은 공식 프로필 확인 3개 그룹·Stray Kids 멤버 8명이다. 아티스트 전용 검수 장소가 없으면 부족 안내와 공통 장소를 표시한다. 선택은 기기/클라우드 snapshot에 보존하며 기존 저장본은 전체 탐색으로 읽는다. [범위](../data/catalog-conditions.md).
