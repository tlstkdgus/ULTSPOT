...
> **2026-09-19 T-006 개정:** 제품은 여러 K팝 그룹·개인·유닛·솔로를 지원한다. 아래 스트레이 키즈·지역 한정은 초기 조사 범위이지 제품 제한이 아니다. 외부 담당자의 [수집 요청서](../data/collection-brief.md)와 [여러 아티스트 지원 계획](multi-artist-data.md)을 적용한다. X API 단독 확보가 아닌 직접 제공·이용 조건을 확인한 공식 자료·공공데이터도 검수 후 활용한다. X 미확보만으로 합법적 확보 경로가 없다고 단정한 기존 가정 7 설명은 정정한다. 가상 행사 제출은 T-005에서 철회했으며 현재 구현 기준은 [실제 플래너 계약](public-planner.md)이다.
