...
test('verbatim conditions preserve evidence and reject broken event/source links', () => {
  ...
  b.assets.push({ asset_id: 'photo', original_url: 'https://example.com/photo.png', source_page_url: 'https://example.com/notice', rights_holder: '미확인', permission_status: '미확인', evidence_reference: '미확인', alt_ko: '검증용 이미지', alt_en: 'Test image', rights_checked_at: '2026-09-20T10:00:00+09:00' });
  b.events[0].image_asset_id = 'photo';
  b.places[0].image_asset_id = 'photo';
  ...
  assert.equal(validate(b).bundle.assets[0].permission_status, '미확인');
  b.assets[0].source_page_url = 'http://example.com';
  b.assets[0].rights_checked_at = '2026-09-20';
  b.events[0].image_asset_id = 'missing';
  assert.deepEqual(validate(b).errors.map(e => e.code).sort(), ['expected_https_url_without_credentials', 'invalid_timestamp', 'unresolved_reference'].sort());
  b.assets[0].source_page_url = 'https://example.com/notice';
  b.assets[0].rights_checked_at = '2026-09-20T10:00:00+09:00';
  b.events[0].image_asset_id = 'photo';
